...
public class DMeeting {
	...
	public List<Interval> getFreeTimeII(List<List<Interval>> schedule, int k) {
		...
		// for k = totalEmployee, which means previous problem, our start is
		// first time we don't have any meeting
		// but for other k smaller than it, our start time is first meeting's
		// start time
		int start = -1;
		if (k < totalEmployee) {
	...
	public static void main(String[] args) {
		...
		for (Interval interval : res) {
			System.out.println("start : " + interval.start + "; end : "
					+ interval.end);
		}
