...
class DFS {
	...
	private Map<String, Integer> costMap; // this two memorization hash map must
											// work together
	private Map<String, Integer> stopMap;
	private Map<String, Map<String, Integer>> map;

	public int findMinCost(List<List<String>> flights, String from, String to,
			int k) {
		this.map = new HashMap<>();
		this.costMap = new HashMap<>();
		this.stopMap = new HashMap<>();
		for (List<String> flight : flights) {
			map.putIfAbsent(flight.get(0), new HashMap<String, Integer>());
			map.putIfAbsent(flight.get(1), new HashMap<String, Integer>());
			map.get(flight.get(0)).put(flight.get(1),
					Integer.parseInt(flight.get(2)));
		}
		for (String city : map.keySet()) {
			this.costMap.put(city, Integer.MAX_VALUE);
			this.stopMap.put(city, Integer.MAX_VALUE);
		}
		dfs(new HashSet<String>(), new ArrayList<String>(), from, to, k, -1, 0);
		return minCost == Integer.MAX_VALUE ? -1 : this.minCost;
	}

	private void dfs(Set<String> visited, List<String> curPath, String cur,
			String to, int k, int stop, int cost) {
...
class Dijk {
	...
	public int findMinCost(List<List<String>> flights, String from, String to,
		...
		Queue<Point> pq = new PriorityQueue<>(1, new Comparator<Point>() {
...
// BFS
// public int findMinCost(List<List<String>> flights, String start, String end,
// int k) {
// // BFS + Queue
...
//
// public int findCheapestPrice(List<List<String>> flights, String src, String
// dst, int k) {
// //follow up
// prev = new HashMap<>();
//
