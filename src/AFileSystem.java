...
public class AFileSystem {
	...
	Map<String, BiConsumer<Integer, Integer>> watchMap;
	...
	public boolean create(String path, int val) {
		...
		if (index == -1 || !map.containsKey(path.substring(0, index))
				&& index != 0) {
			return false;
		}
		map.put(path, val);
	...
	public boolean watch(String path, String message) {
		...
		BiConsumer<Integer, Integer> addition = new BiConsumer<Integer, Integer>() {
